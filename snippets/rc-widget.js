class RechargeWidget extends HTMLElement {
  ...
  async renderWidget() {
    ...
    if (widget.widget_template_type != 'checkbox') {
      ...
      document
        ...
        .addEventListener('change', this.updateRadioWidget.bind(this, widgetTemplate));
    } else {
      this.renderSubscriptionCheckbox(sellingVariant, widgetTemplate);
      // Add change event listeners to the variant id input and update the widget when the variant changes
      document
        .querySelector('.product-variant-id')
        .addEventListener('change', this.updateCheckboxWidget.bind(this, widgetTemplate));
    }
  ...
  renderSubscriptionRadio(sellingVariant) {
    ...
    if (discount) {
      ...
      widgetTemplate.querySelector('.subscription-radio .option-discount').innerText =
        prod.selling_plan_groups[0].selling_plans[0].price_adjustments_value + '%';
    }
    ...
    widgetTemplate
      .querySelector('.subscription-radio .widget__radio-input')
      .addEventListener('change', this.handleSubscriptionChange.bind(this));
  ...
  renderOneTimeRadio(sellingVariant, widgetTemplate) {
    ...
    widgetTemplate
      .querySelector('.one-time-radio .widget__radio-input')
      .addEventListener('change', this.handleOneTimeChange.bind(this));
  }
  ...
  renderSubscriptionCheckbox(sellingVariant, widgetTemplate) {
    ...
    widgetTemplate.querySelector('.option-discount').innerText =
      prod.selling_plan_groups[0].selling_plans[0].price_adjustments_value + '%';
  ...
  renderDetails() {
    if (this.widget.show_subscription_details) {
      ...
      document.querySelector('.widget__subscription-details__text').innerText =
        this.widget.subscription_details_verbiage;
